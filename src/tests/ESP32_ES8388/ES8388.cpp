...
bool ES8388::begin(int32_t sda, int32_t scl, uint32_t frequency)
{
    ...
    if (res == true)
    {
        ...
        res &= write_reg(ES8388_ADDR, ES8388_CONTROL2, 0x00);
        ...
        res &= write_reg(ES8388_ADDR, ES8388_CONTROL1, 0x10);
...
void ES8388::selectInput(uint8_t val)
{
   //  write_reg(ES8388_ADDR, 0x03, 0x08); 
  //  write_reg(ES8388_ADDR, 0x04, 0x3F); // Power up all outputs
     write_reg(ES8388_ADDR, 0x02, 0xf3); //
    write_reg(ES8388_ADDR, 0x0A, 0x50); // LIN2/RIN2 input
     write_reg(ES8388_ADDR, 0x0B, 0x00); // LIN2/RIN2 input
    write_reg(ES8388_ADDR, 0x09, 0x00); // Max analog gain
    write_reg(ES8388_ADDR, 0x10, 0x00); // LADCVOL 0dB
    write_reg(ES8388_ADDR, 0x11, 0x00); // RADCVOL 0dB
    write_reg(ES8388_ADDR, 0x1A, 0x00); // Monitor mode: ADC to all outputs
    write_reg(ES8388_ADDR, 0x1B, 0x00); // Monitor mode: ADC to all outputs
    write_reg(ES8388_ADDR, 0x2E, 0x1E); // LOUT1 0dB
     write_reg(ES8388_ADDR, 0x26, 0x09); // ROUT1 0dB
    write_reg(ES8388_ADDR, 0x27, 0x40); // ROUT1 0dB
    write_reg(ES8388_ADDR, 0x2a, 0x40); // ROUT1 0dB
    write_reg(ES8388_ADDR, 0x2F, 0x1e); // ROUT1 0dB
    write_reg(ES8388_ADDR, 0x30, 0x1e); // LOUT2 0dB
    write_reg(ES8388_ADDR, 0x31, 0x1e); // ROUT2 0dB
    write_reg(ES8388_ADDR, 0x32, 0x1e); // LSPKOUT 0dB
   // write_reg(ES8388_ADDR, 0x02, 0xaa); //
    //write_reg(ES8388_ADDR, 0x35, 0x1E); // RSPKOUT 0dB
    //write_reg(ES8388_ADDR, 0x1C, 0x00); // Unmute DACs

}

uint8_t ES8388::readES8388Register(uint8_t reg) {
  Wire.beginTransmission(ES8388_ADDR);
  ...
  if (Wire.available()) {
    ...
    return 0xFF; // or another error indicator
  }
}

void ES8388::setEqualizer(void) {
 // Left channel, low shelf
 write_reg(ES8388_ADDR,0x1D, 0x1c);  // +6dB gain (try 0x1E if 0x2E doesn't work)
//write_reg(ES8388_ADDR, 30, 0x1A); // Gain: +12?dB
//write_reg(ES8388_ADDR, 31, 0x0b); // Freq: ~100?Hz
//write_reg(ES8388_ADDR, 32, 0x0c); // Freq: ~100?Hz
//write_reg(ES8388_ADDR, 33, 0x01); // Freq: ~100?Hz
   
//write_reg(ES8388_ADDR,34, 0x16);   // Set MIX_CTRL1: bass cutoff to 100Hz and other bits to 0.
//write_reg(ES8388_ADDR,35, 0x11);   // Set TONE_CTRL: treble=0, bass=15 (11.25 dB)
//write_reg(ES8388_ADDR,35, 0x12);   // Set MIX_CTRL1: bass cutoff to 100Hz and other bits to 0.
//write_reg(ES8388_ADDR,37, 0x11);   // Set TONE_CTRL: treble=0, bass=15 (11.25 dB)

//write_reg(ES8388_ADDR,0x21, 0x20);   // Set DAC_CTRL: enable tone control (bit5) and disable other features (like mute, deemph, etc.)
//write_reg(ES8388_ADDR,31, 12); // Left channel, Band 1 freq (~100 Hz)
//write_reg(ES8388_ADDR,34, 46); // Right channel, Band 1 gain
//write_reg(ES8388_ADDR,35, 12); // Right channel, Band 1 freq
}