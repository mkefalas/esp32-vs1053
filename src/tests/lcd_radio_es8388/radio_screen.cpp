...
i2s_config_t i2s_config = {
    ...
    .fixed_mclk = 11289600   // <--- Force standard MCLK for 44.1kHz
};
...
void setup()
{
   ...
   //es.SetVolumeSpeaker(volume);
  //es.SetVolumeHeadphone(volume);

       // audio.setPinout(I2S_BCLK, I2S_LRCK, I2S_SDOUT, I2S_MCLK);
    // audio.setVolume(10); // 0...21
    es.selectInput(2); // 16 bit word length, I2S format; MCLK / Fs = 256
     delay(10000);
 /*   for (int i = 0; i < 0x32; i++)
    {
       uint8_t regValue = es.readES8388Register(i);
  Serial.printf("Value of ES8388 reg :%02d , value:%02x\n", i, regValue);
        delay(10);
    }*/
    //
