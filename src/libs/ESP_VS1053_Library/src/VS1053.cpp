...
/**
 * Switch audio input to LINE1 (default analog input)
 */
void VS1053::switchToLine1() {
    uint16_t mode = read_register(SCI_MODE);
    mode |= _BV(SM_LINE1); // Set LINE1 bit
    writeRegister(SCI_MODE, mode);
    delay(10);
    await_data_request();
    //LOG("Switched to LINE1 input\n");
}

/**
 * Switch audio input to default analog input (normal operation)
 */
void VS1053::switchToDefaultInput() {
    uint16_t mode = read_register(SCI_MODE);
    mode &= ~_BV(SM_LINE1); // Clear LINE1 bit
    writeRegister(SCI_MODE, mode);
    delay(10);
    await_data_request();
    //LOG("Switched to default analog input\n");
}
